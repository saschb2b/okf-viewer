...
| DI1 Structural extraction | Shipped | The helper computes a deterministic, bounded structural classification beside the unchanged per-page text: repeated running lines and margin rails with occurrence counts, numbered and title-case headings with page and line locators, footnotes recognized by carrying a URL or stated date (position deliberately plays no part, because margin artifacts push real footnotes out of any bottom zone), figure and table captions as named gaps, and diagnostics for furniture share, glyph-spacing damage, and truncation. Heavy furniture share and repeated intra-word splits are promoted to adapter-receipt warnings. The DI0 baselines held byte-for-byte: content is unchanged, classification is additive. |
| DI2 The intake plan | Shipped | Rust computes the plan from the structural classification with no model involved: sources are sorted by title and fingerprint so the same selection gives byte-identical plans and a stable SHA-256 plan id, headings at the shallowest level become split points with page-and-line spans, furniture becomes the exclusion list with reasons, footnotes become the unverified evidence inventory, and figures stay named gaps. Plans persist in bounded app data keyed by granted bundle, storing titles and fingerprints only, never a local path; a forged or hostile plan is rejected on save. The dialog ships behind the command palette's Plan document intake, renders with no agent connected, and offers exactly two verbs: keep/drop a proposed concept, and save. Split-point editing is deferred to the package that consumes it. |
| DI3 Guided intake proposals | In progress | The handoff shipped: an intake plan is its own task-launcher origin offering enrichment, creation, and authoring, with the picked documents traveling as ordinary adapted sources and a prompt derived deterministically from the plan alone (kept concepts with spans, furniture that must not travel into concepts, unverified evidence candidates, named gaps). Proposals flow through the existing staged boundary unchanged. Not yet done: uncertainty-first review ordering, span locators inside the persisted evidence records beyond the existing receipt identity, and a benchmark lane proving the completion gate's claim-anchoring and no-furniture properties against a live agent. |
| DI4 Overlap and contradiction | In progress | The deterministic plan-level half shipped: one URL cited by several sources becomes a shared-evidence overlap, one title proposed by several sources becomes a same-topic overlap, and distinct stated dates on shared evidence are listed as a freshness disagreement, both positions per source and nothing merged. Not yet done: claim-level same-topic detection over agent-proposed concepts, conflict findings in the `io.okf.reliability` vocabulary, and the seeded factual-error gate, which need DI3's proposal half to exist first. |
| DI5 Rerun and drift | In progress | Rerun shipped: a saved plan reruns against freshly picked documents through the explicit picker, the diff is pure Rust, each source is named unchanged, changed, new, or missing by refresh-fingerprint comparison, affected concepts are listed by name and nothing else, keep/drop marks survive onto every surviving concept, and an unchanged selection reports nothing to do. Not yet done: staged difference proposals from the report and routine-surfaced drift notices in the attention inbox. |
