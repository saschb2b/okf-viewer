...
- **Inspectable before anything runs.** The plan renders as [structured agent work](structured-agent-work.md): which concepts would exist, which source spans feed each one, what is excluded and why, and which gaps remain. A user can adjust split points and exclusions before committing an agent to the work.
