...
- **Handed off, not executed.** **Start intake thread** carries the plan and its session-held source evidence into the [native task launcher](native-okf-tasks.md) as its own origin, offering enrichment, creation, or authoring. The kickoff prompt is derived from the plan alone: kept concepts with spans, furniture that must not travel, unverified evidence candidates, and gaps no text may be invented for. A reopened saved plan holds identity without content, so starting a thread from it requires choosing the documents again.
- **Saved and re-runnable.** The plan persists beside the bundle with the refresh fingerprint of every source. Rerunning a saved plan asks for the documents again, since no path or content persists, then reports the impact: each source named unchanged, changed, new, or missing by fingerprint comparison, the affected concepts listed by name, and the saved keep/drop marks carried onto every surviving concept. An unchanged selection says so instead of proposing work. The plan is a description of work, never authority: rerunning it requires the same explicit actions as running it.
...
- The plan itself surfaces overlap deterministically, before any agent runs: one URL cited by several sources is shared evidence, one title proposed by several sources is the same topic twice, and distinct stated dates on shared evidence are named as a freshness disagreement. Both positions are listed per source; nothing is merged.
